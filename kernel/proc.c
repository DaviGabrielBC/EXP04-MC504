...
// (bulwark)
int last_scheduled_index = 0; // Índice circular para o escalonamento
...
scheduler(void)
{
  ...
  for(;;){
    ...
    for (i = 0; i < NPROC; i++) {
      int index = (last_scheduled_index + i) % NPROC; // Acessar circularmente
      p = &proc[index];

      acquire(&p->lock);
      if (p->state == RUNNABLE) {
        // Alterne o processo para executar
        p->state = RUNNING;
        c->proc = p;
        last_scheduled_index = index; // Atualizar o índice
        swtch(&c->context, &p->context);

        // Após o retorno, o processo mudou seu estado.
        c->proc = 0;
      }
      release(&p->lock);
    }
